...
export default function Onboarding({ onComplete, loading, error, onClearError }) {
  ...
  const rowBtn = 'w-full text-left px-5 py-3.5 rounded-2xl border text-sm transition-all'
  const rowActive = 'bg-terra-50 dark:bg-terra-900/30 border-terra-300 dark:border-terra-600 text-terra-700 dark:text-terra-300 font-medium'
  const rowIdle   = 'bg-white dark:bg-gray-800 border-cream-300 dark:border-gray-600 text-ink-600 dark:text-gray-300 hover:border-terra-200 dark:hover:border-terra-700'
        ...
        <div className="w-full max-w-lg">
          ...
          {stepName === 'welcome' && (
            <div className="animate-fade-up text-center">
              ...
              <div className="card p-6 mb-8 text-left">
                <AriaSays>Hey! I'm Aria 👋 I'm here to be your personal content strategist — think of me as your guide to growing on TikTok and Reels. I'll build you a custom strategy, content calendar, and post ideas based on <em>your</em> specific situation. Let's start by getting to know you a bit.</AriaSays>
              </div>
          ...
          {stepName === 'name' && (
            <div className="animate-fade-up">
              ...
              <div className="mt-6">
                <input autoFocus value={form.name} onChange={e => set('name', e.target.value)} onKeyDown={e => e.key === 'Enter' && form.name.trim() && next()} placeholder="Your name or creator name..." className="input text-base py-4" />
              </div>
          ...
          {stepName === 'platforms' && (
            <div className="animate-fade-up">
              <AriaSays>Nice to meet you, {form.name}! 🎉 Which platforms are you creating on?</AriaSays>
          ...
          {stepName === 'niche' && (
            <div className="animate-fade-up">
              <AriaSays>What's your content about? Be specific — the more detail, the better your strategy will be.</AriaSays>
              <div className="mt-6 mb-6">
                <textarea autoFocus value={form.niche} onChange={e => set('niche', e.target.value)} placeholder="e.g. I post budget cooking for college students — quick meals under $5 that actually taste good..." rows={4} className="input resize-none leading-relaxed" />
                <p className={clsx('text-right text-xs mt-1.5 font-mono transition-colors', form.niche.length > 15 ? 'text-sage-500' : 'text-ink-300 dark:text-gray-600')}>
          ...
          {stepName === 'followers' && (
            <div className="animate-fade-up">
              <AriaSays>How's your following looking right now? No judgment — this helps me pitch the strategy at the right level. 📊</AriaSays>
              <div className="flex flex-wrap gap-2 mt-6 mb-6">
          ...
          {stepName === 'frequency' && (
            <div className="animate-fade-up">
              <AriaSays>How often are you posting right now? Be honest — I'll build around your real schedule. 📅</AriaSays>
              <div className="flex flex-col gap-2 mt-6 mb-6">
                {FREQ.map(f => (
                  <button key={f} onClick={() => set('postingFreq', f)} className={clsx(rowBtn, form.postingFreq === f ? rowActive : rowIdle)}>{f}</button>
                ))}
              </div>
              <div className="flex gap-3">
                <button onClick={back} className="btn-secondary px-5">←</button>
          ...
          {stepName === 'struggle' && (
            <div className="animate-fade-up">
              <AriaSays>What's your biggest challenge right now? I'll tackle this head-on in your strategy. 💪</AriaSays>
              <div className="flex flex-wrap gap-2 mt-6 mb-6">
          ...
          {stepName === 'goal' && (
            <div className="animate-fade-up">
              <AriaSays>What's the main thing you want to achieve with your content? 🎯</AriaSays>
              <div className="flex flex-col gap-2 mt-6 mb-6">
                {GOALS.map(g => (
                  <button key={g} onClick={() => set('goal', g)} className={clsx(rowBtn, form.goal === g ? rowActive : rowIdle)}>{g}</button>
                ))}
              </div>
              <div className="flex gap-3">
                <button onClick={back} className="btn-secondary px-5">←</button>
          ...
          {stepName === 'review' && (
            <div className="animate-fade-up">
              <AriaSays>Perfect, {form.name}! I've got everything I need. Building your personalised strategy now ✨</AriaSays>
              ...
              {loading
                ? <div className="flex flex-col items-center gap-3 py-4"><Spinner text="Aria is building your strategy…" /><p className="text-xs text-ink-300 dark:text-gray-600 font-mono">Researching trends · crafting your plan</p></div>
                : <div className="flex gap-3"><button onClick={back} className="btn-secondary px-5">Edit</button><button onClick={() => onComplete(form)} className="btn-primary flex-1 py-4">Build my strategy ✦</button></div>
              }
            </div>
          )}
